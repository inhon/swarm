...
class Drone(dronekit.Vehicle):
    ...
    def __init__(self, connection_string):  
        ...
        self.kp=10 # P gain for velocity control
...
def getUnitVector(p1, p2):
    ...
    return: 單位向量 (north, east, down)
    """
    ecef1 = geodetic_to_ecef(*p1)#轉換為ECEF座標系
    ecef2 = geodetic_to_ecef(*p2)
    delta_ecef = ecef2 - ecef1
    R = ecef_to_ned_matrix(*p1)#建立 p1 所在點的 NED 坐標系旋轉矩陣
    delta_ned = R @ delta_ecef #將 ECEF 向量轉換為 NED 座標系
    unit_vector_ned = delta_ned / np.linalg.norm(delta_ned) #正規化為單位向量
    return unit_vector_ned
