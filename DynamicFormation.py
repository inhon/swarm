...
動態轉向隊型跟隨:
1. 隊長（base）持續向一個目標座標飛行
2. 隊形成員根據隊長的機頭方向 + 即時位置，計算rover的目標位置，rover不斷更新飛向目標點的速度(使用PD控制器)
3. 隊形持續保持相對位置與朝向(base的朝向，TODO)，像戰機編隊一樣「轉向同步」
系統設計概念:
每架rover都有：
1.各自的 PD Controller
2.各自的「隊形偏移」（如左後、右後）
3.定時（每 0.5 秒）更新目標位置並追蹤飛行
控制流程:
[base]
1.計算當前位置 + 朝向
2.對所有隊形 offset 套用 yaw 旋轉，得到rover的目標座標(GPS)
3.發送給rover → 各機轉頭(TODO) + 飛行
'''
